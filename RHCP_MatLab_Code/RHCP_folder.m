function [ ] = RHCP_folder(codeFolderName, imageFolderName, resultsFolderName, resolution, PerCut, tolConvergence, numPaths, Mutation, primary_nodes_dist, disp_num, disp_size, annealingTime, numRun, CPMax, num_smoothing,fracParamZr,fracParamZrH,valueZrH, num_bands, bridge_criteria_ratio, plotFrequency, desiredAngle, W, y_step)
%-------------------------------------------------------------------------%
%                                                                         %
...
% Description:
% This function is called by RHCP_main and calls RHCP_file for each 
% microstructure to measure its RHCP.
% Once the analysis is over, it saves in the result folder a .csv file 
% containing the name and RHCP values of all microstructures.
...
for i=1:length(number_vect)
    if strlength(names_vect)>0
        ...
        [RHCP] = RHCP_file(codeFolderName,imageFolderName, char(names_vect(i)), resultsFolderName, resolution, PerCut, tolConvergence, numPaths, Mutation, primary_nodes_dist, disp_num, disp_size, annealingTime, numRun, CPMax, num_smoothing,fracParamZr,fracParamZrH,valueZrH, num_bands, bridge_criteria_ratio, plotFrequency, desiredAngle, W, y_step);
        % Save the results for the final csv file
        results_mat(i,:)=[number_vect(i),RHCP]; 
    end
end
...
% add a header
cHeader = {'Image number' 'RHCP'}; %header
