...
% Last Modified by GUIDE v2.5 28-Jun-2020 15:30:59
...
global g_SizeSpot;
global g_SizeHole;
global g_removeBorders;
global g_nonUniformBackground;
global g_nonUniformBackground_value;
...
%     g_SizeSpot = varargin(3);
%     g_removeBorders = varargin(4);
%     ImageFilename = varargin{5};
try
	...
	elseif numberOfArguments <= 11
		% 3 input arguments will give nargin = 6 for some reason. %, g_removeBorders, g_nonUniformBackground, g_nonUniformBackground_value
		[g_MinThreshold, g_MaxThreshold, g_SizeSpot, g_SizeHole, g_removeBorders, g_nonUniformBackground, g_nonUniformBackground_value, ImageFilename] = deal(varargin{:}); % copies the contents of the cell array X to the separate variables Y1, Y2, Y3, ...
		binsToSuppress = [];
	elseif numberOfArguments == 10
		% Has an extra argument for the bins to supress.
		[g_MinThreshold, g_MaxThreshold, g_SizeSpot, g_SizeHole, g_removeBorders, g_nonUniformBackground, g_nonUniformBackground_value, ImageFilename, binsToSuppress] = deal(varargin{:}); % copies the contents of the cell array X to the separate variables Y1, Y2, Y3, ...
	elseif numberOfArguments == 13
		...
		% respectfully
		[g_MinThreshold, g_MaxThreshold, g_SizeSpot, g_SizeHole, g_removeBorders, g_nonUniformBackground, g_nonUniformBackground_value, ImageFilename, binsToSuppress, threshold2Use] = deal(varargin{:}); % copies the contents of the cell array X to the separate variables Y1, Y2, Y3, ...
		assert( numel(threshold2Use) == 3, 'The threshold2Use must be a 1x2 array.');
        ...
        g_SizeSpot = threshold2Use(3);
	elseif numberOfArguments == 14
	...
	if isa(ImageFilename, 'char')
		...
		if length(imageDimensions) == 3
			...
			set(handles.ColorBand, 'string', 'Initial image');
    ...
    set(handles.nonUniformBackgroundCompensate, 'value', false);
    set(handles.ValueNonUniformBackground, 'string',g_nonUniformBackground_value);
...
varargout{1} = double(g_MinThreshold);
varargout{2} = double(g_MaxThreshold);
varargout{3} = double(g_SizeSpot);
varargout{4} = double(g_SizeHole);
varargout{5} = g_removeBorders;
varargout{6} = g_nonUniformBackground;
varargout{7} = g_nonUniformBackground_value;
varargout{8} = g_LastThresholdedColorBand;
...
try
	...
	if (colorBand ~= 1) || (numberOfColorBands == 1)
        ...
        if g_nonUniformBackground
            % Determine the background
            se = strel('disk',g_nonUniformBackground_value);
            axes1_image = imcomplement(axes1_image);
            background = imopen(axes1_image,se);
            % Remove the background from the image
            axes2_image = axes1_image - background;
            % adjust the intensity of the light
            axes2_image = imadjust(axes2_image);
        ...
        g_removeBorders = get(handles.radRemoveBorders,'Value');
        if g_removeBorders == 1
            % Fill in the body
            mask = imfill(axes2_image, 'holes'); % Mask is whole solid body.
            % XOR the two images
            axes2_image=imcomplement(xor(mask,axes2_image));
        end
...
ShowFinalBinaryImage(hObject, eventdata, handles);
guidata(hObject, handles);
return; % ShowThresholdedBinaryImage

%=====================================================================
function handles = ShowFinalBinaryImage(hObject, eventdata, handles)
global g_MaxThreshold;
global g_MinThreshold;
global g_SizeSpot;
global g_SizeHole;
...
global g_nonUniformBackground_value;
global axes1_image; % Can be a monochrome or color image array of any data type.
global axes3_image; % A uint8 image array.
global g_LastThresholdedColorBand;
global numberOfColorBands;
global axesShowingInMiddleImage;
try
	...
	if (colorBand ~= 1) || (numberOfColorBands == 1)
        ...
        if g_nonUniformBackground
            ...
            background = imopen(axes1_image,se);
            % Remove the background from the image
            axes3_image = axes1_image - background;
            % adjust the intensity of the light
            axes3_image = imadjust(axes3_image);
        ...
        if g_removeBorders == 1
            % Fill in the body
            mask = imfill(axes3_image, 'holes'); % Mask is whole solid body.
            % XOR the two images
            axes3_image=imcomplement(xor(mask,axes3_image));
        end
        % Erase the cluster of less than Nsize pixels (clean the image from isolated
        % groups of pixels)
        ...
        axes3_image = imcomplement(bwmorph(imcomplement(axes3_image),'thin',Inf));
            % Erase the pixels at the intersections between hydrides
            B = bwmorph(imcomplement(axes3_image), 'branchpoints');
            B_loc = find(B);
            axes3_image(B_loc)=1;
            % Erase the cluster of less than Nsize2 pixels (clean the image from isolated
            %groups of pixels)
            Nsize2=2;
            axes3_image = imcomplement(bwareaopen(imcomplement(axes3_image), Nsize2));
            % Determine the number of hydrides
            CC=bwconncomp(imcomplement(axes3_image));
            numberHydrides=CC.NumObjects;
            set(handles.numberhydrideBox,'String',numberHydrides);

            % Thicken the hydrides a little bit for the view on the
            % threshold GUI
            se1 = strel('line',1,90);
            se2 = strel('line',1,0);
            axes3_image = imcomplement(imdilate(imcomplement(axes3_image),[se1,se2],'full'));
		...
		try
			...
			set(handles.txtInfo,'string','Error displaying Axes3');
			errorMessage = sprintf('Error in function %s() at line %d.\n\nError Message:\n%s', ...
				ME.stack(1).name, ME.stack(1).line, ME.message);
			WarnUser(errorMessage);
		end
	...
	axesShowingInMiddleImage = 3;	% Flag that we're showing the binary image.
catch ME
	errorMessage = sprintf('Error in function %s() at line %d.\n\nError Message:\n%s', ...
		ME.stack(1).name, ME.stack(1).line, ME.message);
	WarnUser(errorMessage);
...
guidata(hObject, handles);
return; % from ShowFinalBinaryImage()
%=====================================================================
...
try
	...
	if get(handles.axesHist, 'visible')
		...
		if dblLowThreshold > maxXValue
			dblLowThreshold = double(maxXValue);
		end
...
% --- Executes on button press in radDontRemoveBorders. %%%%%%%%%%%%%%%%%%%%%%%
function radDontRemoveBorders_Callback(hObject, eventdata, handles)
% hObject    handle to radDontRemoveBorders (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% Hint: get(hObject,'Value') returns toggle state of radDontRemoveBorders
...
global g_removeBorders;
try
	set(handles.radDontRemoveBorders, 'value', true); % Set this one true.
...
% --- Executes on button press in radRemoveBorders. %%%%%%%%%%%%%%%%%%%%%%%
function radRemoveBorders_Callback(hObject, eventdata, handles)
% hObject    handle to radRemoveBorders (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% Hint: get(hObject,'Value') returns toggle state of radRemoveBorders
% MATLAB quirk: when you repeatedly click the radio button, it toggles it's value rather than
% keeping it set (selected) as it should.  As a workaround, make sure it gets set (not toggled) and
% make sure the other radio buttons are set to false.
% Change mouse pointer (cursor) to an hourglass.
...
global g_removeBorders;
try
	set(handles.radRemoveBorders, 'value', true); % Set this one true.
	% Set the other radio button's value the opposite of this one.
	set(handles.radDontRemoveBorders, 'value', false);
	ShowFinalBinaryImage(hObject, eventdata, handles);
    g_removeBorders = 1;
catch ME
	errorMessage = sprintf('Error in function %s() at line %d.\n\nError Message:\n%s', ...
...
function edit_spotsize_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_spotsize (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
%=====================================================================
function edit_spotsize_Callback(hObject, eventdata, handles)
% Stub for Callback of the uicontrol handles.edit_spotsize.
global g_SizeSpot;
global g_LastThresholdedColorBand;
try
	g_LastThresholdedColorBand = get(handles.listbox_DisplayColorBand,'Value') - 1;
...
return; % from edit_spotsize_Callback()



function numberhydrideBox_Callback(hObject, eventdata, handles)
% hObject    handle to numberhydrideBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of numberhydrideBox as text
%        str2double(get(hObject,'String')) returns contents of numberhydrideBox as a double


% --- Executes during object creation, after setting all properties.
function numberhydrideBox_CreateFcn(hObject, eventdata, handles)
% hObject    handle to numberhydrideBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function edit_holesize_Callback(hObject, eventdata, handles)
% hObject    handle to edit_holesize (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_holesize as text
%        str2double(get(hObject,'String')) returns contents of edit_holesize as a double
global g_SizeHole;
global g_LastThresholdedColorBand;
try
	g_LastThresholdedColorBand = get(handles.listbox_DisplayColorBand,'Value') - 1;
...
% --- Executes during object creation, after setting all properties.
function edit_holesize_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_holesize (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in nonUniformBackgroundDontCompensate.
function nonUniformBackgroundDontCompensate_Callback(hObject, eventdata, handles)
% hObject    handle to nonUniformBackgroundDontCompensate (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of nonUniformBackgroundDontCompensate
set(gcf,'Pointer','watch');
drawnow;	% Cursor won't change right away unless you do this.
global g_nonUniformBackground;
try
	set(handles.nonUniformBackgroundDontCompensate, 'value', true); % Set this one true.
	% Set the other radio button's value the opposite of this one.
	set(handles.nonUniformBackgroundCompensate, 'value', false);
	ShowFinalBinaryImage(hObject, eventdata, handles);
    g_nonUniformBackground = 0;
...
guidata(hObject, handles);
ShowThresholdedBinaryImage(hObject, eventdata, handles)
return;  % End of nonUniformBackgroundCompensate_Callback()


% --- Executes on button press in nonUniformBackgroundCompensate.
function nonUniformBackgroundCompensate_Callback(hObject, eventdata, handles)
% hObject    handle to nonUniformBackgroundCompensate (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of nonUniformBackgroundCompensate

set(gcf,'Pointer','watch');
drawnow;	% Cursor won't change right away unless you do this.
global g_nonUniformBackground;
try
	set(handles.nonUniformBackgroundCompensate, 'value', true); % Set this one true.
	% Set the other radio button's value the opposite of this one.
	set(handles.nonUniformBackgroundDontCompensate, 'value', false);
	ShowFinalBinaryImage(hObject, eventdata, handles);
    g_nonUniformBackground = 1;
catch ME
	errorMessage = sprintf('Error in function %s() at line %d.\n\nError Message:\n%s', ...
		ME.stack(1).name, ME.stack(1).line, ME.message);
	WarnUser(errorMessage);
...
function ValueNonUniformBackground_Callback(hObject, eventdata, handles)
% hObject    handle to ValueNonUniformBackground (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ValueNonUniformBackground as text
%        str2double(get(hObject,'String')) returns contents of ValueNonUniformBackground as a double
global g_nonUniformBackground_value;
try
    % Get the value from the control.
	ValueNonUniformBackground_value = str2double(get(handles.ValueNonUniformBackground,'string'));
    if (ValueNonUniformBackground_value >= 1)
        g_nonUniformBackground_value = ValueNonUniformBackground_value;
    end
...
% --- Executes during object creation, after setting all properties.
function ValueNonUniformBackground_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ValueNonUniformBackground (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
