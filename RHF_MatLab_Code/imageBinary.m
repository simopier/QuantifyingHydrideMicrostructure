function [ binaryImage,lowThreshold, highThreshold, SpotSize, HoleSize, removeBorders, background_bool, background_value ] = imageBinary(codeFolderName, ImageFolderName,filename, startingLowThreshold, startingHighThreshold, SpotSize, HoleSize, resultsFolderName )
...
% Erase the pixels at the intersections between hydrides, as well as their
% direct neighbors
B = bwmorph(binaryImage, 'branchpoints');
[B_loc_x,B_loc_y] = find(B);
D = bwdistgeodesic(binaryImage,B_loc_y,B_loc_x,'quasi-euclidean');
D_loc = find(D<=1);
binaryImage(D_loc)=0;
